...
import { Message } from '@/interface/Inertia';
...
const DropdownMessage = () => {
  ...
  const [unreadMessages, setUnreadMessages] = useState<Message[] | null>(null);
  ...
  useEffect(() => {
    api
      ...
      .then((result: AxiosResponse<Message[]>) => {
  ...
  return (
    <li className="relative" x-data="{ dropdownOpen: false, notifying: true }">
      ...
      <div
        ...
        className={`absolute -right-16 mt-2.5 flex max-h-90 min-h-[120px] w-75 flex-col rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark sm:right-0 sm:w-80 ${
        ...
        <ul className="flex h-full flex-col overflow-y-auto">
          {unreadMessages &&
            unreadMessages.map((message) => (
              <li key={message.id}>
                <Link
                  className="flex gap-4.5 border-t border-stroke px-4.5 py-3 hover:bg-boxgray-2 dark:border-strokedark dark:hover:bg-meta-4"
          ...
          {!unreadMessages?.length && (
            <li className="h-30">
              <div className="flex h-full items-center justify-center">
                <span className="text-sm">There are no unread messages...</span>
              </div>
            </li>
          )}
        </ul>
